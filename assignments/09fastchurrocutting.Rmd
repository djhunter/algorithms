...
```{python}
price = [0,1,5,8,9,10,17,17,20,24,30,34,34,38,40,45,46,47,50,52,55]
...
def FastCutChurro(n):
  ...
  for j in range(1,n+1):
    ...
    for i in range(1,j+1):
      totPriceTry = price[i] + maxPrice[j-i]
      if totPriceTry > maxPrice[j]:
        maxPrice[j] = totPriceTry
  return maxPrice[n]

print(FastCutChurro(20))
```
...
while(n > 0) {
  left <- n-bills
  left[which(left<0)] <- Inf
  change[[i]] <- bills[which.min(left)]
  n <- n - change[[i]]
  i <- i + 1
}
```
